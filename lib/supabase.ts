...
// Club-wide functions for managers
export const getClubPosts = async (clubId: string, postType: 'organization' | 'coach' = 'organization') => {
  const { data, error } = await supabase
    .from('posts')
    .select(`
      *,
...
export const getClubEvents = async (clubId: string, startDate?: string, endDate?: string) => {
  let query = supabase
    .from('events')
    .select(`
      *,
